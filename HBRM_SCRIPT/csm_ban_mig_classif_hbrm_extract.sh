...
CONN_STR=`echo $1 | tr '[:lower:]' '[:upper:]'`
...
###########################################################################################
# Dropping the temporary table after getting over with our business
###########################################################################################
...
spool $TLG_CSM_ROOT/interface/output/drop_temp_table_after_${USER}_${INST}_${DATE_TIME}.log
...
if [ `grep ORA- $TLG_CSM_ROOT/interface/output/drop_temp_table_after_${USER}_${INST}_${DATE_TIME}.log | wc -l` -gt 0 ]
then

      echo "Error in Dropping Temporary Table TEMP_BAN_HBRM_DTS"
      grep ORA- $TLG_CSM_ROOT/interface/output/drop_temp_table_after_${USER}_${INST}_${DATE_TIME}.log
      Exit_Program -1
fi
