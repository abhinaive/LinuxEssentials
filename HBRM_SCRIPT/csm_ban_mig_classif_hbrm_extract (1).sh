...
EncryptOutputFiles()
{
    ...
    FILES_TO_ENC="DSD708_HBRM_${DATE_TIME}*"
      ...
      do           
            ...
            #redirecting checksum of .csv to a file
            touch abhinav_checksum.txt
            eval `sha256sum $f` >> abhinav_checksum.txt
            if [ $? -eq 0 ]
            then
                 echo "checksum for $f successfully appended in abhinav_checksum.txt"
            else
                 echo "failed to append checksum for $f in abhinav_checksum.txt"
                 Exit_Program -1
            fi
